...
async function listarTurnos(req, res) {
    ...
    try {
        ...
        res.json({ page: parseInt(page), limit: parseInt(limit), data: turnos });
    } catch (error) {
...
async function actualizarTurno(req, res) {
    ...
    try {
        const turno = await actualizar(id, status);
        if (!turno) return res.status(404).json({ error: "TURNO NO ENCONTRADO O STATUS INVÁLIDO" });
